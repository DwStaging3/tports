...
opt_acceptsum = False
opt_maint = "unknown <tports@local>"
opt_tdata = {}
opt_nolock = False
...
def init_early():
    ...
    global tbpath, rtpath

    tbpath = os.path.dirname(os.path.realpath(__file__))
    rtpath = os.path.dirname(tbpath)
    ...
    def do_exit(signum, stack):
        from tbuild.core import errors

        raise errors.TbuildException("interrupted!")
...
def handle_options():
    ...
    parser = argparse.ArgumentParser(
        description="Tartarus Linux build system.",
    ...
    parser.add_argument(
        ...
        const=True,
        default=opt_verbose,
        help="Verbose output (from tbuild and builds if supported).",
    )
    parser.add_argument(
        "command",
        nargs="+",
    ...
    if "build" in global_cfg:
        ...
        opt_cchpath = bcfg.get("tbuild_cache_path", fallback=opt_cchpath)
...
def init_late():
    ...
    from tbuild.core import paths, spdx
    from tbuild.apk import sign, util as autil
    ...
    # apk command
    if "TBUILD_APK_PATH" in os.environ:
        paths.set_apk(os.environ["TBUILD_APK_PATH"])
    else:
        paths.set_apk(opt_apkcmd)

    # bwrap command
    if "TBUILD_BWRAP_PATH" in os.environ:
        paths.set_bwrap(os.environ["TBUILD_BWRAP_PATH"])
    else:
...
def short_traceback(e, log):
    ...
    import subprocess
    import shlex
    from tbuild.core import pkg as pkgm
...
def pkg_run_exc(f):
    from tbuild.core import template, errors, logger
    ...
    try:
        ...
        return False, False
    except errors.TbuildException as e:
        log.out(f"\f[red]tbuild: {e!s}")
        if e.extra:
...
def binary_bootstrap(tgt):
    from tbuild.core import chroot, paths
...
def do_unstage(tgt, force=False):
    from tbuild.core import chroot, stage
...
def bootstrap(tgt):
    ...
    from tbuild.core import build, chroot, logger, template, paths
    from tbuild.apk import cli
    ...
    if not chroot.chroot_check(True, False):
        logger.get().out("tbuild: bootstrapping stage 0")
    ...
    if not chroot.chroot_check(True, False):
        logger.get().out("tbuild: bootstrapping stage 1")
        ...
        try:
            do_pkg("pkg", "main/base-tbuild", False, False, stage=1)
        except template.SkipPackage:
            pass
        # go back to stage 1
        paths.reinit_buildroot(oldmdir, 1)
    ...
    if not chroot.chroot_check(True, False):
        logger.get().out("tbuild: bootstrapping stage 2")
        # use stage 1 build root to build, but build into stage 2 repo
        paths.reinit_buildroot(oldmdir, 1)
        chroot.chroot_check(True)
        try:
            do_pkg("pkg", "main/base-tbuild", False, False, stage=2)
        except template.SkipPackage:
            pass
        # go back to stage 2
        paths.reinit_buildroot(oldmdir, 2)
    ...
    if not chroot.chroot_check(True, False):
        logger.get().out("tbuild: bootstrapping stage 3")
        # use stage 1 build root to build, but build into stage 2 repo
        paths.reinit_buildroot(oldmdir, 2)
        chroot.chroot_check(True)
        try:
            do_pkg("pkg", "main/base-tbuild", False, stage=3)
        except template.SkipPackage:
            pass
        # go back to stage 3
        paths.reinit_buildroot(oldmdir, 3)
...
def bootstrap_update(tgt):
    from tbuild.core import chroot
    from tbuild.util import flock
...
def do_clean(tgt):
    ...
    from tbuild.core import paths, chroot, template, logger
...
def do_zap(tgt):
    ...
    from tbuild.core import paths, errors

    if paths.bldroot().is_dir():
        shutil.rmtree(paths.bldroot())
    elif paths.bldroot().exists():
        raise errors.TbuildException("broken build container")
...
def do_prune_obsolete(tgt):
    from tbuild.core import logger, paths
    from tbuild.apk import cli

    logger.get().out("tbuild: pruning repositories...")
...
def do_prune_removed(tgt):
    ...
    from tbuild.core import chroot, logger, paths, template, errors
    from tbuild.apk import cli
...
def do_index(tgt):
    ...
    from tbuild.core import chroot, logger, paths, errors
    from tbuild.apk import cli
    ...
    if idir:
        ...
        if not (repo / archn).is_dir():
            raise errors.TbuildException(f"repository '{repo}' does not exist")
        _index(repo)
        return
    # all repos
    reposd = paths.repository()
...
def do_lint(tgt):
    from tbuild.core import chroot, template
...
def _graph_prepare():
    ...
    from tbuild.core import chroot, template, errors
...
def do_prune_sources(tgt):
    from tbuild.core import chroot, logger, template, errors, paths
    import shutil
    import re
    ...
    for f in paths.sources().iterdir():
        if f.name == "by_sha256" or f.name == "tbuild.lock":
            continue
...
def do_relink_subpkgs(tgt):
    from tbuild.core import chroot, paths, logger, errors, template
    import shutil
...
def do_print_build_graph(tgt):
    ...
    if len(cmdline.command) < 2:
        raise errors.TbuildException("print-build-graph needs a package name")
...
def _get_unbuilt(outdated=False):
    from tbuild.core import chroot, template, paths
    from tbuild.apk import util
    import subprocess
...
def do_update_check(tgt):
    from tbuild.core import update_check, template, chroot
...
def do_pkg(tgt, pkgn=None, force=None, check=None, stage=None):
    from tbuild.core import build, chroot, template, errors, paths
    from tbuild.util import compiler
    ...
    if tgt == "invoke-custom":
        if len(cmdline.command) != 3:
            raise errors.TbuildException(f"{tgt} needs two arguments")
        ...
        if len(cmdline.command) <= 1 and tgt != "chroot":
            raise errors.TbuildException(f"{tgt} needs a package name")
        elif len(cmdline.command) > 2:
            raise errors.TbuildException(f"{tgt} needs only one package")
        if len(cmdline.command) > 1:
...
def _bulkpkg(pkgs, statusf, do_build, do_raw, version):
    ...
    from tbuild.core import logger, template, chroot, errors, build
    ...
    for pn in pkgs:
        ...
        if not pns:
            if pns is None:
                ...
                log.out(f"\f[red]tbuild: missing package '{pn}'")
                failed = True
                continue
            else:
                badpkgs.add(pn)
                statusf.write(f"{pn} invalid\n")
                log.out(f"\f[red]tbuild: invalid package '{pn}'")
                failed = True
                continue
        ...
        if failed and not opt_bulkcont:
            ...
            log.out(f"\f[red]tbuild: skipping template '{npn}'")
            continue
        # finally add to set
        rpkgs.add(npn)

    # visited "intermediate" templates, includes stuff that is "to be done"
    # which will be force-added after base-tbuild is handled, as we don't
    # want this to influence the graph of base-tbuild
    #
    ...
    # this is visited stuff in base-tbuild, we do checks against it later
    # to figure out if to add the implicit base-tbuild "dep" in the topology
    ...
    def handle_recdeps(pn, tp, cbinit=False):
        ...
        if do_raw:
            return True
        # add base-tbuild "dependency" for correct implicit sorting
        if not cbinit and pn not in cbvisit and pn != "main/base-tbuild":
            depg.add(pn, "main/base-tbuild")
    ...
    # first add base-tbuild, since that's an "implicit" dependency
    ...
    for pn in rpkgs:
        ...
        if failed and not opt_bulkcont:
            ...
            log.out(f"\f[red]tbuild: skipping template '{pn}'")
            continue
    ...
    if failed:
        raise errors.TbuildException("at least one bulk package failed")
    elif not opt_stage and do_build is True:
        do_unstage("pkg", False)


def _collect_git(expr):
    from tbuild.core import errors, git
    import pathlib
    ...
    revs = git.call(cmd)
    if revs is None:
        raise errors.TbuildException(f"failed to resolve commits for '{oexpr}'")
...
def do_bulkpkg(tgt, do_build=True, do_raw=False, version=False):
    ...
    if do_raw:
        if len(cmdline.command) <= 1:
            raise errors.TbuildException("need at least one template")
...
def do_prepare_upgrade(tgt):
    from tbuild.core import template, chroot, build, errors
    import pathlib

    if len(cmdline.command) < 2:
        raise errors.TbuildException("prepare-upgrade needs a package name")
...
def do_bump_pkgver(tgt):
    from tbuild.core import chroot, logger, template, errors
    from tbuild.apk import cli as acli
    import pathlib

    if len(cmdline.command) != 3:
        raise errors.TbuildException("bump-pkgver needs a name and a version")

    pkgn = cmdline.command[1]
    pkgv = cmdline.command[2]

    if not acli.check_version(pkgv):
        raise errors.TbuildException(f"version '{pkgv}' is invalid")
...
def do_bump_pkgrel(tgt):
    ...
    if len(cmdline.command) < 2:
        raise errors.TbuildException("bump-pkgrel needs at least one name")
...
def do_commit(tgt):
    from tbuild.core import errors, chroot, paths, template, logger, git
    import tempfile
    import os
    ...
    if gits is None:
        raise errors.TbuildException("failed to resolve git changes")
...
def do_interactive(tgt):
    ...
    from tbuild.core import logger
    ...
    while True:
        pmpt = shlex.split(input("tbuild> "))
        try:
            cmdline = parser.parse_intermixed_args(pmpt)
        except SystemExit:
            logger.get().out("\f[red]tbuild: invalid command line")
            continue
        fire_cmd()
...
def fire_cmd():
    ...
    def bodyf():
        ...
        if cmd in command_handlers:
            ...
            logger.get().out(f"\f[red]tbuild: invalid target {cmd}")
            sys.exit(1)
        return None
...
def fire():
    ...
    from tbuild.core import build, chroot, logger, template, profile
    from tbuild.core import paths
    from tbuild.apk import cli
    from tbuild.util import flock
    ...
    if not opt_signkey and cmdline.command[0] != "keygen":
        logger.get().out("\f[red]tbuild: no signing key set")
        sys.exit(1)
    ...
    if opt_arch:
        try:
            ...
            logger.get().out(
                f"\f[red]tbuild: unknown target architecture '{opt_arch}'"
            )
            sys.exit(1)
    ...
    try:
        ...
        logger.get().out(
            f"\f[red]tbuild: apk not found (expected path: {paths.apk()})"
        )
        sys.exit(1)

    if not aret.stdout.startswith(b"apk-tools 3"):
        logger.get().out("\f[red]tbuild: apk-tools 3.x is required")
        sys.exit(1)
    ...
    try:
        ...
        logger.get().out(
            f"\f[red]tbuild: bwrap not found (expected path: {paths.bwrap()})"
        )
        sys.exit(1)
