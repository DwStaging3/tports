from tbuild.core import chroot, logger
...
def _invoke_svc(pkg):
    ...
    for f in pkg.destdir.glob("usr/lib/dinit.d/*"):
        # for dinit-tartarus we only care about providing targets
        if pkg.rparent.pkgname == "dinit-tartarus" and not f.name.endswith(
            ".target"
        ):
            continue
        _scan_svc(f, "svc")
