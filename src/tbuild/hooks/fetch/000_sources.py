...
def invoke(pkg):
    ...
    dhdrs = {
        "User-Agent": "tbuild-fetch/4.20.69",
        "Accept": "*/*",
    }
    ehdrs = os.getenv("TBUILD_FETCH_HEADERS")
    if ehdrs:
    ...
    caenv = os.getenv("TBUILD_FETCH_CAFILE")
    if caenv:
    ...
    if errors > 0:
        pkg.error(
            ...
            hint="use 'tbuild prepare-upgrade' to update checksums",
        )
