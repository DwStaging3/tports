...
from tbuild.core import logger, chroot, paths, profile, spdx, errors, git
from tbuild.util import compiler, flock
from tbuild.apk import cli, util as autil
...
class Template(Package):
    ...
    def init_from_mod(self):
        ...
        if self.build_style:
            importlib.import_module(
                f"tbuild.build_style.{self.build_style}"
            ).use(self)
        ...
        if self.broken:
            ...
            self.broken = f"cannot be built, disallowed by tbuild (not in {', '.join(_allow_cats)})"
    ...
    def get_build_deps(self):
        from tbuild.core import dependencies
    ...
    def setup_paths(self):
        ...
        self.statedir = bdirbase / (".tbuild-" + self.pkgname)
        ...
        self.env["TBUILD_STATEDIR"] = "/builddir/.tbuild-" + self.pkgname
        ...
        for sp in self.subpkg_list:
            ...
            if not tpath.is_symlink():
                self.error(
                    ...
                    hint="run 'tbuild relink-subpkgs' to rebuild them",
                )
    ...
    def do(
        ...
        *args,
        ...
        cenv = {
            "TBUILD_TARGET_MACHINE": cpf.arch,
            "TBUILD_TARGET_SYSROOT": str(cpf.sysroot),
            "TBUILD_HOST_MACHINE": chroot.host_cpu(),
        }
        ...
        with self.profile("host") as hpf:
            ...
            if hpf.triplet:
                cenv["TBUILD_HOST_TRIPLET"] = hpf.triplet
        ...
        if self.use_ltocache:
            lld_args += [
                ...
                "--thinlto-cache-dir=/tbuild_cache/lld_thinlto_cache",
            ]
...
def sanitize_pkgname(pkgname, error=True):
    ...
    if not tmplpath.is_file():
        if not error:
            return None
        raise errors.TbuildException(f"missing template for '{pkgname}'")
