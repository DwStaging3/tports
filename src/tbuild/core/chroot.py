...
from tbuild.core import logger, paths, errors
from tbuild.apk import cli as apki, sign as signi
...
def chroot_check(force=False, error=True):
    ...
    if (paths.bldroot() / ".tbuild_chroot_init").is_file():
        _chroot_ready = True
        cpun = (paths.bldroot() / ".tbuild_chroot_init").read_text().strip()
        set_host(cpun)
    else:
        _chroot_ready = False
...
def _prepare_etc():
    ...
    with open(tfp / "passwd", "a") as pf:
        pf.write("tbuild:x:1337:1337:tbuild user:/tmp:/bin/nologin\n")

    with open(tfp / "group", "a") as pf:
        pf.write("tbuild:x:1337:\n")

    # machine-id for tbuild
    ...
    # delete potential previous tbuild file so we are clean
    (paths.bldroot() / "usr/lib/sysusers.d/tbuild.conf").unlink(missing_ok=True)
...
def _prepare():
    sfpath = paths.bldroot() / ".tbuild_chroot_init"
    if sfpath.is_file():
        return
    if not (paths.bldroot() / "usr" / "bin" / "sh").is_file():
        raise errors.TbuildException("bootstrap not installed, can't continue")
...
def shell_update(rnet, dirty):
    ...
    with rfile.open("w") as rfh:
        ...
        if rnet:
            from tbuild.core import profile
    ...
    (paths.tbuild_cache() / "apk" / hcpu).mkdir(parents=True, exist_ok=True)
    cfile.symlink_to(f"/tbuild_cache/apk/{hcpu}")
    ...
    with flock.lock(flock.apklock(hcpu)):
        if (
            ...
            raise errors.TbuildException("failed to update pkg database")
...
def install():
    ...
    from tbuild.core import template

    logger.get().out(f"tbuild: installing {' '.join(_extra_pkgs)}...")
    ...
    if irun.returncode != 0:
        raise errors.TbuildException("failed to install container packages")

    logger.get().out("tbuild: container set up successfully!")
...
def get_fakeroot(bootstrap):
    inp = paths.tbuild() / "misc/fakeroot.sh"

    if bootstrap:
        return inp

    rp = paths.bldroot() / ".tbuild_fakeroot.sh"

    if rp.is_file():
        return "/.tbuild_fakeroot.sh"
...
def _setup_dummy(rootp, archn):
    ...
    logger.get().out(f"tbuild: installing virtual provider for {archn}...")
    ...
    atomic_ver = _get_ver("libatomic-tartarus")
    ...
    provides = [
        ...
        f"libatomic-tartarus={atomic_ver}",
        f"libatomic-tartarus-devel={atomic_ver}",
        f"musl={musl_ver}",
...
def _prepare_arch(prof, dirty):
    ...
    if rootp.exists() and not dirty:
        logger.get().out(f"tbuild: clearing sysroot for {prof.arch}...")
        shutil.rmtree(rootp)
...
def cleanup_world(bootstrapping, prof=None, perform=True):
    ...
    if bootstrapping is None:
        bootstrapping = not (paths.bldroot() / ".tbuild_chroot_init").is_file()

    log = logger.get()

    log.out("tbuild: cleaning world...")
    ...
    if f_ret.returncode != 0:
        raise errors.TbuildException("failed to remove autodeps")
...
def update(pkg):
    ...
    logger.get().out(
        f"tbuild: updating software in {paths.bldroot()!s} container..."
    )
    ...
    with flock.lock(flock.apklock(prof.arch)):
        if (
            ...
            raise errors.TbuildException("failed to update cross pkg database")
...
def enter(
    ...
    *args,
    ...
    envs = {
        ...
        "SHELL": "/bin/sh",
        "HOME": "/tmp",
        "USER": "root" if fakeroot else "tbuild",
        "LC_COLLATE": "C",
        "LANG": "C.UTF-8",
        **env,
    }
    ...
    if "TBUILD_STATEDIR" in envs:
        envs["PATH"] = envs["TBUILD_STATEDIR"] + "/wrappers:" + envs["PATH"]
    ...
    if mount_tbuild_cache:
        bcmd += ["--bind", paths.tbuild_cache(), "/tbuild_cache"]

    # always bubblewrap as tbuild user
    ...
    if lldargs:
        ...
        os.close(wfd)
        fdlist.append(rfd)
        bcmd += ["--ro-bind-data", str(rfd), "/tmp/tbuild-lld-args"]
