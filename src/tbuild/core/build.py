from tbuild.core import chroot, logger, dependencies, profile, scanelf, paths
from tbuild.core import template, update_check as uc, pkg as pkgm, errors
from tbuild.util import flock
from tbuild.apk import cli as apk, generate as apkgen, sign as asign
...
def register_hooks():
    for stepn in hooks:
        dirn = paths.tbuild() / "hooks" / stepn
        if dirn.is_dir():
            for f in dirn.glob("*.py"):
                # this must be skipped
                if f.name == "__init__.py":
                    continue
                modn = "tbuild.hooks." + stepn + "." + f.stem
...
def _restricted_importer(name, globals=None, locals=None, fromlist=(), level=0):
    ...
    if (
        ...
        if name != "tbuild.util":
            raise ImportError(
                f"only modules from tbuild.util allowed in template (got: '{name}')"
            )
...
def invoke_fetch(pkg):
    srclock = paths.sources() / "tbuild.lock"
...
def _split_auto(pkg, done):
    ...
    for apkg, adesc, iif, takef in template.autopkgs:
        ...
        if pkg.pkgname == iif:
            continue
        if apkg == "dinit-links" and pkg.rparent.pkgname == "dinit-tartarus":
            continue
...
def _build_locked(
    ...
    with (
        ...
        if ret.returncode != 0:
            raise errors.TbuildException("failed to generate packages")
        ...
        for repo in pkg._stage:
            ...
            if not apk.build_index(repo, pkg.source_date_epoch):
                raise errors.TbuildException("indexing repositories failed")
