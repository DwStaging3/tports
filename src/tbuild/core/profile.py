from tbuild.core import paths, chroot, errors
...
def get_hardening(prof, tmpl, hlist=None):
    ...
    for fl in tmpl.hardening + (hlist if hlist else []):
        ...
        if fl not in hdict:
            raise errors.TbuildException(f"unknown hardening option {fl}")
    ...
    if hdict["cfi"]:
        if not tmpl.options["lto"]:
            raise errors.TbuildException("CFI requires LTO")
        if not hdict["vis"]:
            raise errors.TbuildException("CFI requires hidden visibility")
...
class Profile:
    def __init__(self, archn, pdata, gdata, native=False):
        ...
        if "triplet" not in pdata:
            raise errors.TbuildException(f"unknown triplet for {archn}")

        if "endian" not in pdata:
            raise errors.TbuildException(f"unknown endianness for {archn}")

        if "wordsize" not in pdata:
            raise errors.TbuildException(f"unknown wordsize for {archn}")
...
def init(cparser):
    ...
    for pf in profiles.glob("*.ini"):
        ...
        if archn != "bootstrap" and "profile" not in cp:
            raise errors.TbuildException(f"malformed profile: {archn}")
