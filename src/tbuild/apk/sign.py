from tbuild.core import logger, paths, errors, git
...
def keygen(size, eaddr, cfgfile, cfgpath):
    if not shutil.which("openssl"):
        raise errors.TbuildException("openssl is missing")
    ...
    if keypath.is_file():
        raise errors.TbuildException("attempt to overwrite an existing key")
    ...
    if not kout.returncode == 0:
        raise errors.TbuildException("key generation failed")
    ...
    if not pout.returncode == 0:
        raise errors.TbuildException("public key generation failed")
