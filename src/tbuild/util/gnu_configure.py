...
def configure(
    ...
    cachedir = paths.tbuild() / "misc/autoconf_cache"
...
def replace_guess(pkg):
    for f in pkg.srcdir.rglob("*config*.*"):
        ...
        if f.suffix == ".guess":
            f.unlink()
            shutil.copy(paths.tbuild() / "misc/config.guess", f)
        elif f.suffix == ".sub":
            f.unlink()
            shutil.copy(paths.tbuild() / "misc/config.sub", f)
